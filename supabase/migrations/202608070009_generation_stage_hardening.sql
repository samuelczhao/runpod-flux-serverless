...
begin
  if p_request_hash !~ '^[0-9a-f]{64}$' or nullif(trim(p_operation_key), '') is null
  ...
  if v_job.dream_id <> p_dream_id or v_job.scene_version_id is distinct from p_scene_version_id
    or v_job.stage <> p_stage or v_job.model <> p_model or v_job.request_hash <> p_request_hash then
...
begin
  ...
  if (v_job.stage = 'anchor' and (v_ordinal <> 1 or v_version.parent_version_id is not null))
    or (v_job.stage = 'scene' and (v_ordinal not between 2 and 3 or v_version.parent_version_id is not null))
