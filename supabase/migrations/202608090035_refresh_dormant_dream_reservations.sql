...
begin
  ...
  if v_dream.status not in ('DRAFT', 'UPLOADED')
    or v_dream.workflow_run_id is not null
    or v_dream.workflow_claim_token is not null then return; end if;

  v_current_bucket := (now() at time zone 'UTC')::date;
  v_reserved_bucket := (v_dream.quota_reserved_at at time zone 'UTC')::date;
  if v_reserved_bucket = v_current_bucket then return; end if;
  perform public.increment_dream_quota_usage(p_user_id);
  update public.dreams set quota_reserved_at = now() where id = p_dream_id;
end;
$$;
...
begin
  ...
  if v_dream.status in ('DRAFT', 'UPLOADED') then
